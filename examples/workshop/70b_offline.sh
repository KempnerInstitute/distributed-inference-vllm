...
srun -N 1 -n 1 --mem=512GB -w "$head_node" ray start --head --node-ip-address="$head_node_ip" \
...
for (( i = 1; i <= worker_num; i++ )); do
    ...
    srun -N 1 -n 1 --mem=512GB -w "$node" ${SINGULARITY_WRAP} ray start --address="$head_addr" \
...
srun -N 1 -n 1 --mem=256GB --gpus=0 vllm serve /n/netscratch/kempner_dev/Everyone/models/Llama-3.1-70B --tensor-parallel-size 4 &

python offline.py --model-path=/n/netscratch/kempner_dev/Everyone/models/Llama-3.1-70B --input-file=offline_input.json --output-file=offline_output.json --num-threads=20
